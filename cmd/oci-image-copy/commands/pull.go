...
func pullCmd(parent *cobra.Command, flagConfig *string) {
	var (
		flagLocal              string
		flagRemote             string
		...
		flagAssumeLocalHas     []string
		flagKeepGoing          bool
		flagVerifyReusedBlobs  bool
	)
	...
	f.BoolVar(&flagVerifyReusedBlobs, "verify-reused-blobs", false,
...
func runPull(
	...
	res, err := share.Pull(ctx, ociimagecopy.PullArgs{
		...
		VerifyReusedBlobs: flagVerifyReusedBlobs,
	})
