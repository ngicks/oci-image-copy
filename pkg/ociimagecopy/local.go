...
// operations. The zero value passes no compression flags, so skopeo falls back
// to its own default; the package-wide default (zstd, level 20, forced) lives in
// [DefaultConfig] and flows in through the config layer — it is not re-applied by
// the constructors here. Levels are algorithm-specific (zstd 1-20, gzip 1-9).
...
// NewSkopeoWithCompression builds a [*skopeo.Skopeo] over invoker, applying the
// destination compression settings from cfg verbatim. It does NOT inject a
// default: the zstd default is defined once, in [DefaultConfig], and reaches
// here through the config layer. The local endpoint and the SSH-backed remote
// both go through this one function so their skopeo configuration cannot diverge.
