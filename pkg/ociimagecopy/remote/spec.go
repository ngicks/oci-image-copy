...
// spec.go parses the URI-style --remote flag value. It lives in package remote
// (co-located with the factories it feeds — NewSSH, NewFileServerFromSpec,
// NewLocalDir) to avoid an import cycle: package remote already imports
// ociimagecopy, and RemoteSpec embeds *FileServerRemoteSpec, so keeping the
// remote-spec types in core would force ociimagecopy to import remote. The
// local-spec parser stays in ociimagecopy (LocalSpec / ParseLocalSpec), which
...
//	  file-server:<url>            (fully implemented; see NewFileServerFromSpec)
...
// The factory [NewFileServerFromSpec] constructs a fully functional
...
//
//	file-server:<url>
//	    Fully implemented. [NewFileServerFromSpec] constructs the Remote.
