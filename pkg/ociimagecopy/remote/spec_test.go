...
// spec_test.go — tests for the remote-spec parser (ParseRemoteSpec, redaction
// helpers, ParseChunkSize). These cases were originally in
// pkg/ociimagecopy/spec_test.go and have been moved here because the types
// (ParseRemoteSpec, RemoteKind*, DefaultChunkSize, RedactFileServerURL,
// RedactHeader, ParseChunkSize, FileServerRemoteSpec) now live in package
// remote (spec.go), not in package ociimagecopy.
...
// containsAnySpec is a local helper for spec_test.go (mirrors the one in
// spec_test.go of the parent package, kept local to avoid cross-file naming
// conflicts).
func containsAnySpec(s string, subs ...string) bool {
...
func TestParseRemoteSpec_FileServer_ErrorRedaction(t *testing.T) {
	...
	msg := err.Error()
	if containsAnySpec(msg, "secret", "token=abc") {
